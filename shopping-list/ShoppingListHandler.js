...
function updateItem(filter, body) {
  ...
  return ShoppingList.findOneAndUpdate(
    ...
    { $set: update },
    { new: true }
  ).then(doc => doc.items.id(filter['items._id']));
}

function update(filter, body) {
  return ShoppingList.findOneAndUpdate(filter, { $set: body }, { new: true });
}

function deleteItem(filter) {
  return ShoppingList.findOneAndUpdate(filter, {
...
function addItem(filter, body) {
  return ShoppingList.findOneAndUpdate(
    ...
    { $push: { items: body } },
    { new: true }
  ).then(doc => doc.items[doc.items.length - 1]);
}
...
function remove(filter) {
  return ShoppingList.deleteOne(filter).catch(err =>
